...
const SocialSecurityBase: React.FC<{ active: boolean }> = ({ active }) => {
  ...
  const loadData = async (page = 1, pageSize = 10) => {
    setLoading(true);
    try {
      const res = await request(getApiUrl('/api/soo/social-security-base/page', 'SOO'), { params: { pageNum: page, pageSize } });
  ...
  const handleModalOk = async () => {
    try {
      ...
      let res;
      if (editing) {
        res = await request(getApiUrl(`/api/soo/social-security-base/${editing.id}`, 'SOO'), { method: 'PUT', data: submitData });
      } else {
        res = await request(getApiUrl('/api/soo/social-security-base', 'SOO'), { method: 'POST', data: submitData });
      }
      if (res && res.resp_code === 0) {
  ...
  const handleDelete = async (record: any) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const res = await request(getApiUrl(`/api/soo/social-security-base/${record.id}`, 'SOO'), { method: 'DELETE' });
          if (res && res.resp_code === 0) {
  ...
  const columns = [
    ...
    { title: '社保基数', dataIndex: 'socialBase', key: 'socialBase' },
    { title: '公积金基数', dataIndex: 'fundBase', key: 'fundBase' },
  ...
  return (
    ...
    >
      ...
      />
      <Modal
        title={editing ? '编辑社保公积金基数' : '新增社保公积金基数'}
        open={modalOpen}
        onOk={handleModalOk}
