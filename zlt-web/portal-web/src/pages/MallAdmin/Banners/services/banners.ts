...
export async function createBanner(banner: Omit<BannerDTO, 'id'>): Promise<boolean> {
  const response = await request<ApiResponse<boolean>>('/api-mall/api/mall/admin/banner', {
    method: 'POST',
    data: banner,
  });
...
export async function deleteBanner(id: number): Promise<boolean> {
  const response = await request<ApiResponse<boolean>>(`/api-mall/api/mall/admin/banner/${id}`, {
    method: 'DELETE',
  });
  return response.datas || false;
}
...
export async function updateBannerSort(id: number, sort: number): Promise<boolean> {
  const response = await request<ApiResponse<boolean>>(`/api-mall/api/mall/admin/banner/${id}/sort/${sort}`, {
    method: 'PUT',
  });
  return response.datas || false;
}
