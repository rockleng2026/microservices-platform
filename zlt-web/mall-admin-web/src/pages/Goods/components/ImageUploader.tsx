...
const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const handleUpload = async (file: File): Promise<void> => {
    ...
    try {
      ...
      // Upload to file-center
      const response = await fetch('/file-center/api/upload', {
        method: 'POST',
        body: formData,
      });
      ...
      if (result.resp_code === 0 && result.datas?.url) {
        const newUrls = [...imageUrls, result.datas.url];
        updateUrls(newUrls);
        message.success('上传成功');
      } else {
        message.error(result.resp_msg || '上传失败');
      }
    } catch (error) {
