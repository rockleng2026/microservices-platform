...
## 4. 数据库设计
 @zlt-business/saleops-optimizer/sql/central_soo_all.sql

<!-- ### 4.4 复用现有模块表结构

#### 4.4.1 项目管理数据复用
本模块不重新创建项目相关表，而是复用现有项目管理模块的表结构：

**复用表说明**:
- `project` - 项目基本信息表（项目ID、名称、类别、负责人、参与人员等）
- `project_profit_distribution` - 项目人员毛利分配表（员工角色、分配比例、审批状态等）
- `project_closure` - 项目结项表（结项时间、合同金额、毛利润等）
- `product_profit_distribution_guide` - 产品分配指导表（不同产品类型的标准分配比例）

**数据关联方式**:
- `soo_monthly_performance.team_project_id` 关联 `project.id`
- 通过 `project.participants` 字段（JSON格式）获取项目成员信息
- 通过 `project_profit_distribution` 表获取具体的分配比例和金额
- 通过 `project_closure` 表获取项目的财务数据用于提成计算

#### 4.4.2 操作日志统一管理
本模块不单独维护操作日志表，而是使用平台公共模块提供的统一日志服务：

**日志记录方式**:
- 使用统一的日志切面(AOP)自动记录关键操作
- 通过日志服务API主动记录重要业务操作
- 日志数据存储在公共日志库中，支持跨模块查询和统计

**关键操作日志记录范围**:
- 薪酬配置的增删改操作（部门分红、职级薪资、员工配置）
- 绩效数据的录入和修改操作
- 工资计算和审批操作
- 盈亏平衡分析的生成和修改
- 重要配置参数的变更（社保基数、地区系数等） -->
